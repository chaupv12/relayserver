...
// task scheduler:
	...
	// 'console' output:
	virtual BaseEnv& operator<<(char const* str) = 0;
	virtual BaseEnv& operator<<(int i) = 0;
	virtual BaseEnv& operator<<(unsigned u) = 0;
	virtual BaseEnv& operator<<(double d) = 0;
	virtual BaseEnv& operator<<(void* p) = 0;
...
protected:
	BaseEnv(TaskScheduler& scheduler); // abstract base class
	virtual ~BaseEnv(); // we are deleted only by reclaim()
