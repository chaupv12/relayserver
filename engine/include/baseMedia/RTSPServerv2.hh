...
class RTSPServerv2 : public GenericMediaServer {
public:
	static RTSPServerv2* createNew(BaseEnv& env, Port ourPort = 554, UserAuthenticationDatabase* authDatabase = NULL, unsigned reclamationSeconds = 65);
	...
	static Boolean lookupByName(BaseEnv& env, char const* name, RTSPServerv2*& resultServer);

	typedef void (responseHandlerForREGISTER)(RTSPServerv2* rtspServer, unsigned requestId, int resultCode, char* resultString);
	...
	typedef void (responseHandlerForDEREGISTER)(RTSPServerv2* rtspServer, unsigned requestId, int resultCode, char* resultString);
...
protected:
	RTSPServerv2(BaseEnv& env, int ourSocketIPv4, int ourSocketIPv6, Port ourPort, UserAuthenticationDatabase* authDatabase, unsigned reclamationSeconds);
	// called only by createNew();
	virtual ~RTSPServerv2();
	...
	virtual void implementCmd_REGISTER(char const* cmd/*"REGISTER" or "DEREGISTER"*/, char const* url, char const* urlSuffix, int socketToRemoteServer, Boolean deliverViaTCP, char const* proxyURLSuffix);
	// used to implement "RTSPClientConnection::handleCmd_REGISTER()"
...
public: // should be protected, but some old compilers complain otherwise
  // The state of a TCP connection used by a RTSP client:
	...
	public:
		...
		public:
			ParamsForREGISTER(char const* cmd/*"REGISTER" or "DEREGISTER"*/, RTSPClientConnection* ourConnection, char const* url, char const* urlSuffix, Boolean reuseConnection, Boolean deliverViaTCP, char const* proxyURLSuffix);
	...
	protected:
		RTSPClientConnection(RTSPServerv2& ourServer, int clientSocket, struct sockaddr_storage const& clientAddr);
		virtual ~RTSPClientConnection();

		friend class RTSPServerv2;
		...
		virtual void handleCmd_REGISTER(char const* cmd/*"REGISTER" or "DEREGISTER"*/, char const* url, char const* urlSuffix, char const* fullRequestStr, Boolean reuseConnection, Boolean deliverViaTCP, char const* proxyURLSuffix);
	...
	protected:
		...
		RTSPServerv2& fOurRTSPServer; // same as ::fOurServer
	...
	protected:
		RTSPClientSession(RTSPServerv2& ourServer, u_int32_t sessionId);
	...
	protected:
		...
		RTSPServerv2::RTSPClientConnection* fOurClientConnection;
...
class RTSPServerWithREGISTERProxying : public RTSPServerv2 {
public:
	static RTSPServerWithREGISTERProxying* createNew(BaseEnv& env, Port ourPort = 554,
...
protected:
	RTSPServerWithREGISTERProxying(BaseEnv& env, int ourSocketIPv4, int ourSocketIPv6, Port ourPort,
